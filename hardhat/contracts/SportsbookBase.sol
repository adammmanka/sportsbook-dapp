...
contract SportsbookBase {
    ...
    ChallengeResult[] public challengesResult;
    ...
    function completeChallenge(
        ...
        require(
            matchChallenges[_challengeId].team1 != address(0) &&
                matchChallenges[_challengeId].team2 != address(0),
            "One team empty (or canceled match)!"
        );
        require(
            msg.sender == matchChallenges[_challengeId].team1 ||
                msg.sender == matchChallenges[_challengeId].team2,
            "You must be one of the teams to say who won"
        );
        // Effect
        matchChallenges[_challengeId].finished = true;
        challengesResult.push(
            ChallengeResult(_challengeId, _team1Result, _team2Result)
        );
        // Interact
